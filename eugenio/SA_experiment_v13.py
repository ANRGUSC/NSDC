...
import DAG_generator_scaled_v5_one_sink
...
for s in number_of_tasks:
    ...
    for t in range(0,1): # creation of a task graph
        ...
        #number of nodes.  
        G_dag,dag=DAG_generator_scaled_v5_one_sink.random_dag(nodes[t]-1,'n')
        plt.figure(0)
        nx.draw(G_dag)
        #--------   
...
for k in N_nodes:
    ...
    #x0=ran.randrange(f)
    x0=temp_H_list
    opt = sa_v5.minimize(info,optimization_sa_v2, x0,bounds, opt_mode='continuous', step_max=2000, t_max=1, t_min=0)

...
class minimize():
    ...
    def __init__(self, info,optimization_sa_v1, x0, bounds, opt_mode, cooling_schedule='linear', step_max=1000, t_min=0, t_max=100, alpha=None, damping=1):
        ...
        self.info = info
        self.cost_func = optimization_sa_v1
        self.x0 = x0
        self.bounds = bounds[:]
        ...
        temp = self.cost_func.optimization(self.x0,self.info.Connectivity_Matrix,self.info.Network_N_matrix,self.info.number_of_tasks,self.info.bandwith_table,self.info.user_input,self.info.Cost_Matrix,self.info.DAG_matrices,self.info.dags)
        ...
        while self.step < self.step_max and self.t >= self.t_min and self.t>0:
            ...
            temp_energy=self.cost_func.optimization(proposed_neighbor,self.info.Connectivity_Matrix,self.info.Network_N_matrix,self.info.number_of_tasks,self.info.bandwith_table,self.info.user_input,self.info.Cost_Matrix,self.info.DAG_matrices,self.info.dags)
            E_n = temp_energy[0]
            dE = E_n[0] - self.current_energy[0]

...
"""
nodes=np.arange(4,6,2) # number of nodes per task graph
#-------------------------
...
for t in range(0,len(nodes)): # loop for different task graphs of "n" nodes
    ...
    for i in range(0,len(dag)):
            for j in range(0,len(dag[i])):
                #if len(dag[i])!=0:
                DAG_matrix[i][int(dag[i][j])]=8#ran.randrange(1,20) #amount of data transferred [data units]
...
# This is DATA that the user provides:
#-------------------------
N=6 # main graph number of nodes/agents

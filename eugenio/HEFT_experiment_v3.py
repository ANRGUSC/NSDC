...
import core as cor
import Network_N
import DAG_generator_scaled
import time

# USER SET UP :
#-------------

number_of_tasks=np.arange(4,8,2) # number of task graphs

# mother network node sizes:
min_number_of_nodes=100
max_number_of_nodes=501
step=100
...
for s in number_of_tasks:
    ...
    for t in range(0,1): # creation of a task graph
        ...
        G_dag,dag=DAG_generator_scaled.random_dag(nodes[t], edges[t])
        #plt.figure(0)
        #nx.draw(G_dag)
        #--------   
        ...
        for i in range(0,len(dag)):
                for j in range(0,len(dag[i])):
                    #if len(dag[i])!=0:
                    DAG_matrix[i][int(dag[i][j])]=ran.randrange(1,20) #amount of data transferred [data units]
    ...
    for r in range(0,Number_of_graphs): # different Network's loop
        ...
        for t in range(0,1): # loop for the different task graphs
            ...
            def compcost(job, agent):
                ...
                comp_cost=DAG_matrix[int(job)][int(job)] / Network_N_matrix[agent][agent]
            ...
            def commcost(ni, nj, agent_A, agent_B):
                ...
                if(agent_A == agent_B):
                    ...
                    comm_cost = DAG_matrix[int(ni)][int(nj)] / Network_N_matrix[agent_A][agent_B] 
                    return comm_cost

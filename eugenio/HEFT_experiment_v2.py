...
import core as cor
import Network_N
import DAG_generator_v1
import time

# USER SET UP :
#-------------

number_of_tasks=np.arange(10,15,1) # number of task graphs

# mother network node sizes:
min_number_of_nodes=100
max_number_of_nodes=105
step=1
...
for s in number_of_tasks:
    ...
    for t in range(0,s): # creation of different task graphs
        ...
        G_dag,dag=DAG_generator_v1.random_dag(nodes[t], edges[t])
        #plt.figure(0)
        #nx.draw(G_dag)
        #--------   
    ...
    for r in range(0,Number_of_graphs): # different Network's loop
        ...
        for t in range(0,s): # loop for the different task graphs

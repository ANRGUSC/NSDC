...
number_of_tasks=np.arange(101,102,2) # number of task graphs / minimum is 6 !!

# mother network node sizes:
min_number_of_nodes=100
max_number_of_nodes=201
step=50
...
for i in range(0,N):
    Network_N_matrixa[i][i]=ran.randrange(1,5)# agent's speed [cycles/sec]
...
for i in range(0,N):
    for j in range(i,N):# we work with a complete graph. we fill up everything
        ...
        if i!=j:
            Connectivity_Matrixa[i][j]=ran.randrange(1,4)#number of edges(RF/gray/Satellite)
                                                        #per pair of nodes
...
for k in N_nodes:
    ...
    #x0=ran.randrange(f)
    x0=temp_H_list
    opt = sa_v5.minimize(info,optimization_sa_v2, x0,bounds, opt_mode='continuous', step_max=20000, t_max=1, t_min=0)
...
filename = '101DAGs_100to200Net_20000SAsteps.pkl'

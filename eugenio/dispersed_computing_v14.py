...
class all_subgraphs: # list with each subgraph of N and each possible connectivity 
                       # choice
...
# ----------------
# Here we generate the different possible subgraphs of each Network N
...
for i in range(0,N):
    for j in range(i,N):# we fill up everything in case we work with a complete graph
...
for i in range(0,len(H_list)):
    ...
    for c in product(list_, repeat = len(edges_)):
        results.append(c)
    #comb=combinations(range(int(temp_sum)),len(edges_))
    ...
    # only have an RF connection; thus, a permutation result(1,1,1) is not 
    # possible because that would mean that each edge has RF/gray (0 and 1) choice.
    ...
    #Network_N_matrix[i][j]=2# bandwith of the edges between agents [data/sec]
...
for r in range(0,sum(number_of_subgraphs)): # different Network's loop
    
    # Here we fill up de Network_N_matrix with the different bandwiths
